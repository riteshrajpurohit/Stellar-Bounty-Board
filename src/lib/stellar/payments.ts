...
function parseHorizonError(error: unknown): string {
  ...
  if (resultCodes) {
    ...
    if (txResult === 'tx_insufficient_fee') return 'Insufficient fee. Please try again.';
    if (txResult === 'tx_bad_auth') return 'Bad signature — did you reject in Freighter?';
    if (txResult === 'tx_insufficient_balance' || opResults.includes('op_underfunded')) {
    ...
    if (opResults.length > 0) return `Operation failed: ${opResults.join(', ')}`;
  }
...
export const createAndSendPayment = async ({
  ...
  try {
    ...
    throw new Error(
      msg.toLowerCase().includes('user') || msg.toLowerCase().includes('cancel') || msg.toLowerCase().includes('reject')
  ...
  // Freighter v6 always returns { signedTx: string }
  const signedXdr: string =
    typeof signedResponse === 'string'
      ? signedResponse
      : signedResponse?.signedTx ?? signedResponse?.xdr ?? null;
  ...
  if (!submitResult.successful) {
    throw new Error('Transaction was submitted but Horizon reported it as unsuccessful.');
  }
